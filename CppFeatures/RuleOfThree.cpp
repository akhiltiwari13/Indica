...
template <class T> void printc(T t) {
  for (auto itr : t) {
  	std::cout << itr << " ";
  }
  cout << endl;
...
public:
  ...
  Resource &operator=(const Resource &ref) {
    /* When you write a copy assignment operator, values are going into an
...
private:
  ...
  string lastname;
  int arbitrarynumber;
  /* unique_ptr<Resource> pResource; //the Person objects will have a solid
   * Resouce object. */
  shared_ptr<Resource>
      pResource; // the Person objects will have a solid Resouce object.
public:
  Person(string fname, string lname, int arbitrary)
      : firstname(fname), lastname(lname), arbitrarynumber(arbitrary) {
    cout << "Constructing Person " << fname << " " << lname << endl;
  }
  ...
  void SetResource(string name) { pResource = make_shared<Resource>(name); }
  string GetResourceName() const { return pResource->GetName(); }

  /* for shared pointer, copy constructor definition and copy assignment
   * operator definition would not be necessary. */
...
int main() {
  // boost::progress_timer timer; // as it goes out of scope, it prints the time
  // the timer was in scope.
  ...
  /* Person p2 = p; //when creating a new object using assignment operator, a
   * copy constructor is invoked and not the copy assignment operator. */
  /* p=p2; */
  ...
  REP(0, 5) { vec.push_back(i); }
  printc(vec);

  for_each(begin(vec), end(vec), [=](int element) mutable {
    x += element;
    y += element;
  });
